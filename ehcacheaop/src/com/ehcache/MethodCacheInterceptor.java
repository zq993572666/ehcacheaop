...
public class MethodCacheInterceptor implements MethodInterceptor,
	...
	public MethodCacheInterceptor(EhCacheCacheManager methodCache){
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		if (cache == null) {
			...
			result = invocation.proceed();// 这个就是调用数据访问方法，如这里是调用manager.PersonManagerImpl.getList(),并用result保存执行的结果（数据访问的结果）,如这里调用了getList()方法，会先打印出"get Person from DB"
											// ，然后将结果集放入到result里面去，这里由于使用的是自己配置只能放入10个元素的ehcache，所以这里的result是ArrayList<E>
											// ，它里面存放的是elementData[10]，并将getList得到的结果放入到elementData里面去了
			...
			Cache addcache = new Cache("test1", 150, true, false, 5, 2);
			cacheManager.addCache(addcache);
			element = new Element(cacheKey, (Serializable) result);
			addcache.put(element);// 放入addcache中
	...
	public void afterPropertiesSet() throws Exception {
		/*
		 * if(null == cache) { throw new
		 * IllegalArgumentException("Cache should not be null."); }
		 */
	}
